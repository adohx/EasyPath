...
void main() {
  group('searchPlaces', () {
    test('returns places from the backend when results are present',
        () async {
      ...
      final places =
          await ApiService.withClient(client).searchPlaces('library');
    ...
    test('falls back to Nominatim when the backend returns no results',
        () async {
      final client = MockClient((request) async {
    ...
    test(
        'falls back to local mock data when the backend and Nominatim '
        'both fail', () async {
      ...
      check(places).length.equals(2);
      check(places.first.id).equals('mock_001');
    });
  });
  ...
  group('planRoutes', () {
    test('returns routes parsed from the backend response', () async {
      final client = MockClient((request) async {
        ...
        check(request.url.host).equals(_backendHost);
        return _json({'routes': [_sampleRouteJson()]});
      });
    ...
    test('falls back to mock routes when the backend fails', () async {
      final client = MockClient((request) async => http.Response('boom', 500));
      ...
      check(routes.map((r) => r.id)).deepEquals(['route_001', 'route_002']);
    });
  });
  ...
  group('nearbyExploration', () {
    test('returns categories parsed from the backend response', () async {
      ...
      final categories = await ApiService.withClient(client)
    ...
    test('falls back to mock exploration data when the backend fails',
        () async {
      final client = MockClient((request) async => http.Response('boom', 500));
      ...
      check(categories.map((c) => c.key)).contains('restaurant');
    });
  });
}
